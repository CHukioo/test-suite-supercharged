...
test.describe('Purchase Flow Tests', () => {
  ...
  test('should navigate to the purchase page after choosing a flight', async ({ page }) => {
    ...
    await compareFullPageSnapshot(page, 'purchase-page.png');
  });
  ...
  test('should display the purchase form with all required fields', async ({ page }) => {
    ...
    await waitForImagesToLoad(page);
    await compareFullPageSnapshot(page, 'purchase-form-fields.png');
  });
  ...
  test('should complete a full purchase and show confirmation', async ({ page }) => {
    ...
    // Mask the dynamic confirmation ID before snapshotting
    await hideDynamicContent(page, ['table td:first-child']);
    await waitForImagesToLoad(page);
    await compareFullPageSnapshot(page, 'confirmation-page.png');
  });
});
